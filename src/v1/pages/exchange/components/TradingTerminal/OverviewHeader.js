...
import { subscribeToSocket } from 'v1/services/socket'
import { ENDPOINT_URL } from 'v1/config'
import { numberWithCommas } from 'v1/utils/utils'
  ...
  return (
    ...
    >
      <section className={classes.overviewHeaderContent}>
        <Link to="/markets" className={classes.overviewHeaderLink}>
          <ChevronLeftIcon />
        </Link>
        <Grid container direction="column" justify="flex-start">
          <Typography className={classes.stockTitle} variant="h1">
            {data && data.name}
          </Typography>
      ...
      <Grid container direction="column" justify="flex-end" item xs={6}>
        <Typography className={classes.price} variant="h4" align="right">
          {data?.quote?.numberFormat?.currency}{" "}
