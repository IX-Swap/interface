...
import { GraphQLArgs, PoolsRepositoryFetchOptions } from '@ixswap1/dex-v2-sdk'
...
export default function usePoolsQuery(
      ...
      fetch: async (options: PoolsRepositoryFetchOptions): Promise<Pool[]> => {
        ...
        if (shouldInjectTokens) {
          const tokens = flatten(
            pools.map((pool) => [...pool.tokensList, ...tokenTreeLeafs(pool.tokens), pool.address])
          )
          injectTokens(tokens)
        }
  ...
  function initializeDecoratedSubgraphRepository() {
    return {
      fetch: async (options: PoolsRepositoryFetchOptions): Promise<Pool[]> => {
        const pools = await balancerSubgraphService.pools.get(getQueryArgs(options))

        const poolDecorator = new PoolDecorator(pools)
        let decoratedPools = await poolDecorator.decorate(tokenMeta)
  ...
  function getQueryArgs(options: PoolsRepositoryFetchOptions): GraphQLArgs {
    const { tokens, poolIds, poolTypes, sortField, poolAttributes } = filterOptions
    ...
    const tokensListFilterOperation = filterOptions.useExactTokens ? 'eq' : 'contains'

    const tokenListFormatted = tokens?.map((address) => address.toLowerCase()) || []

    const orderBy = isBalancerApiDefined ? sortField || 'totalLiquidity' : 'totalLiquidity'
    ...
    if (queryArgs.where && hasPoolIdFilters) {
      queryArgs.where.id = { in: filterOptions.poolIds }
    }
    if (options.first) {
    ...
    if (queryArgs.where && hasPoolAttributeFilters && poolAttributes.includes(PoolAttributeFilter.New)) {
  ...
  function customSort(pools: Pool[]): Pool[] {
    const poolsSortField = filterOptions.sortField || 'totalLiquidity'
  ...
  const queryFn = async ({ pageParam = 0 }) => {
    ...
    try {
      ...
      const savedPools = poolsStoreService.pools
  ...
  options.getNextPageParam = (lastPage: PoolsQueryResponse) => lastPage.skip || 0
