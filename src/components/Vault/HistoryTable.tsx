...
import { ActionHistoryStatus, isTransaction } from './enum'
...
export const HistoryTable = ({ currency }: { currency?: Currency }) => {
  ...
  const isTransactionRow = useMemo(() => isTransaction(eventLog[0]?.type), [eventLog])
  return (
    <>
      {eventLog.length > 0 && (
        <table style={{ marginTop: '26px', width: '100%', border: 'none' }} cellSpacing="0" cellPadding="0">
          <HistoryHeader currencySymbol={currency?.symbol ?? ''} isTransaction={isTransactionRow} />
          {currency &&
            eventLog.map((row) => {
