      ...
      <Block23>
        {config?.footerConfig?.block2 || (
          <>
            <div>
              <Trans>
                {config?.name || 'IX Swap'} is a globally accessible decentralized finance platform which allows for
                exchange of security tokens and cryptocurrency. At {config?.name || 'IX Swap'}, we look to offer
            ...
            <div>
              <Trans>
                Your use of the {config?.name || 'IX Swap'} involves various risks, including, but not limited to,
                losses while digital assets are being supplied to the {config?.name || 'IX Swap'} protocol and losses
                due to the fluctuation of prices of tokens in a trading pair or liquidity pool. Before using{' '}
                {config?.name || 'IX Swap'}, you should review the relevant documentation to make sure you understand
                how {config?.name || 'IX Swap'} works.
              </Trans>
            </div>
          </>
        )}
          ...
          <>
            <div>
              <Trans>
                ...
                KIND. {config?.name || 'IX Swap'} platform is run by smart contracts deployed on the Ethereum
                blockchain. Upgrades and modifications to the protocol are managed in a community-driven way by holders
                of the {config?.name || 'IX Swap'} governance token. No developer or entity involved in creating the{' '}
                {config?.name || 'IX Swap'} will be liable for any claims or damages whatsoever associated with your
                use, inability to use, or your interaction with other users of, the IX Swap platform, including any
            ...
            <div>
              <Trans>
                The {config?.name || 'IX Swap'} Platform is not meant for US investors or other FATF prohibited
                jurisdictions, potentially including Iran, Afghanistan, North Korea, Hong Kong, Singapore, and/or other
                jurisdictions. Users must pass sufficient KYC checks in order to use the {config?.name || 'IX Swap'}{' '}
                platform.
              </Trans>
            </div>
          </>
