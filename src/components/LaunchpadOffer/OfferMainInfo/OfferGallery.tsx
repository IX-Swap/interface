...
export const OfferGallery: React.FC<Props> = (props) => {
  ...
  const cardImage = React.useMemo(() => ({ file: props.offer.cardPicture, type: OfferFileType.image, videoUrl: '' }), [])
    ...
    <GalleryContainer>
      <GalleryCarousel>
        ...
        <GalleryCarouselExtraMediaList>
          ...
          {gallery.length > 3 && <GalleryCarouselExtra onClick={() => openViewer()}>+{gallery.length - 3}</GalleryCarouselExtra>}
        </GalleryCarouselExtraMediaList>
      </GalleryCarousel>
      ...
      <SocialMediaLinks>
        ...
        {socialMedialLinks
          ...
          .map((link, idx) => <SocialMediaLink key={`link-${idx}`} href={link.url}>{link.logo}</SocialMediaLink>)
        }
      </SocialMediaLinks>
    </GalleryContainer>
  )
