...
export class Web3Helpers {
  ...
  constructor() {
    this.web3 = new Web3(
      new Web3.providers.HttpProvider(`https://${process.env.REACT_APP_INFURA_NETWORK_SUBDOMAIN}.infura.io/v3/${process.env.REACT_APP_INFURA_KEY}`)
    )
  }
  ...
  async getTokenBalance(tokenAddress: string | undefined, walletAddress: any) {
    ...
    const decimals = await contract.methods.decimals().call(function (error: any, d: any) {
      return d
    })
  ...
  async sendCrypto(
    wallet: { ethAddress: any; privateKey: string },
    addressTo: any,
    amount: { toString: () => import('bn.js') }
  ) {
    try {
      // create transaction
