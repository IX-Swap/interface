...
import LoginForm, { loginFormInitialValues } from 'v2/Auth/Login/Login'
...
describe('LoginForm', () => {
  ...
  it('handles submit', async () => {
    ...
    } = renderWithUserStore(<LoginForm />, { login })
    const email = getByPlaceholderText(/email address/i)
    const password = getByPlaceholderText(/password/i)
    const loginButton = getByText(/login/i)
  ...
  it('handles click on "Forgot Password?"', async () => {
    const { getByText } = renderWithUserStore(<LoginForm />)
    const forgotPasswordButton = getByText(/forgot password/i)
