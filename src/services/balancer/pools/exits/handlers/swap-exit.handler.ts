import { getTimestampSecondsFromNow } from 'hooks/dex-v2/useTime'
import { fetchPoolsForSor, hasFetchedPoolsForSor } from 'lib/balancer.sdk'
import { bnum, formatAddressForSor, selectByAddress } from 'lib/utils'
import { Pool } from 'services/pool/types'
import { BalancerSDK, SwapInfo, SwapType } from '@ixswap1/dex-v2-sdk'
import { TransactionResponse } from '@ethersproject/abstract-provider'
import { BigNumber, formatFixed, parseFixed } from '@ethersproject/bignumber'
import { JsonRpcSigner } from '@ethersproject/providers'
import { ExitParams, ExitPoolHandler, ExitType, QueryOutput } from './exit-pool.handler'
...
export class SwapExitHandler implements ExitPoolHandler {
  ...
  async exit(params: ExitParams): Promise<TransactionResponse> {
    ...
    if (!this.lastSwapRoute) throw new Error('Could not fetch swap route for join.')
   ...
   */
  private async queryOutGivenIn({ bptIn, tokenInfo, amountsOut, signer }: ExitParams): Promise<QueryOutput> {
    const amountIn = bptIn
    const tokenIn: any = selectByAddress(tokenInfo, this.pool.address)

    const tokenOut: any = tokenInfo[amountsOut[0].address]

    if (!tokenIn || !tokenOut) throw new Error('Missing critical token metadata.')
    ...
    const amountOut = formatFixed(this.lastSwapRoute.returnAmount, tokenOut.decimals)
    if (bnum(amountOut).eq(0)) throw new Error('Not enough liquidity.')

    const priceImpact = this.calcPriceImpact(amountIn, amountOut, this.lastSwapRoute.marketSp)
   ...
   */
  private async queryInGivenOut({ tokenInfo, amountsOut, signer }: ExitParams): Promise<QueryOutput> {
    const tokenIn: any = selectByAddress(tokenInfo, this.pool.address)
    const tokenOut: any = selectByAddress(tokenInfo, amountsOut[0].address)
    if (!tokenIn || !tokenOut) throw new Error('Missing critical token metadata.')

    const amountOut = amountsOut[0].value
    if (!amountOut || bnum(amountOut).eq(0)) return { amountsOut: {}, priceImpact: 0, txReady: true }

    if (!hasFetchedPoolsForSor) await fetchPoolsForSor()

    const safeAmountOut = overflowProtected(amountsOut[0].value, tokenOut.decimals)
    ...
    const amountIn = formatFixed(this.lastSwapRoute.returnAmount, tokenIn.decimals)
  ...
  private calcPriceImpact(amountIn: string, amountOut: string, marketSp: string): number {
  ...
  private getSwapAttributes(exitType: ExitType, swapInfo: SwapInfo, maxSlippage: number, userAddress: string) {
    const deadline = getTimestampSecondsFromNow(60).toString() // 60 seconds from now
    const kind = exitType === ExitType.GivenIn ? SwapType.SwapExactIn : SwapType.SwapExactOut
