...
import { BalancerSDK, SwapInfo } from '@ixswap1/dex-v2-sdk'
...
export class SwapJoinHandler implements JoinPoolHandler {
  ...
  async join(params: JoinParams): Promise<TransactionResponse> {
    ...
    const { to, data, value } = this.getSwapAttributes(this.lastSwapRoute, slippageBsp, userAddress)
  ...
  async queryJoin({ amountsIn, tokensIn, signer }: JoinParams): Promise<QueryOutput> {
    if (amountsIn.length === 0) throw new Error('Missing amounts to join with.')
    ...
    if (!amountIn.value || bnum(amountIn.value).eq(0)) return { bptOut: '0', priceImpact: 0 }
    ...
    const bptOut = formatFixed(this.lastSwapRoute.returnAmount, this.pool.onchain?.decimals || 18)
    if (bnum(bptOut).eq(0)) throw new Error('Not enough liquidity.')

    const priceImpact = this.calcPriceImpact(amountIn.value, bptOut, this.lastSwapRoute.marketSp)
  ...
  private getSwapAttributes(swapInfo: SwapInfo, maxSlippage: number, userAddress: string) {
