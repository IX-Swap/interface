...
import { getCookiesForAllAccounts, postRequest } from '../lib/helpers/api'
import { baseCreds } from '../lib/helpers/creds'
import { sellOrder, buyOrder } from '../lib/helpers/api-body'
let cookiesUser_1,
  cookiesUser_2,
  cookiesUser_3,
  getBalancesBefore,
...
if (baseCreds.URL.includes('dev')) {
  test.beforeAll(async () => {
    ;[cookiesUser_1, cookiesUser_2, cookiesUser_3] =
      await getCookiesForAllAccounts()
  })

  test.beforeEach(async ({ page, investment }) => {
    getBalancesBefore = await investment.fullBalances([
      cookiesUser_1,
      cookiesUser_2,
      cookiesUser_3
    ])
  ...
  test.describe('Between 2 users', () => {
    test('Full match order (BUY first)', async ({ investment, auth }) => {
      ...
      await test.step('Check that funds have been distributed', async () => {
        const getBalancesAfter = await investment.fullBalances([
          cookiesUser_1,
          cookiesUser_2,
          cookiesUser_3
        ])
    ...
    test('(Cancel BUY order) The user must receive the remaining money back', async ({
      investment,
      auth
    }) => {
      buyOrder['price'] = 10
      buyOrder['amount'] = 6
      orderResponse = await postRequest(
        buyOrder,
        cookiesUser_3,
        text.requests.orderExchange
      )
      await test.step('The buy order should be created', async () => {
        await auth.loginWithout2fa(baseCreds.secondExchange, baseCreds.PASSWORD)
        await investment.toSecondaryMarket(text.requests.TOKEN_SGD_PAIR)
    ...
    test('(Cancel SELL order) The user must receive the remaining money back', async ({
      investment,
      auth
    }) => {
      await test.step('The sell order should be created', async () => {
    ...
    test('Full match order (SELL with price that is lower than BID Price)', async ({
      investment,
      auth
    }) => {
      await test.step('The buy order should be created', async () => {
      ...
      await test.step('Check that funds have been distributed', async () => {
        ...
        expect(getBalancesBefore.user2SGDBalance.outstanding).toEqual(
          getBalancesAfter.user2SGDBalance.outstanding - 40
        )
        expect(getBalancesBefore.user3SGDBalance.outstanding).toEqual(
          getBalancesAfter.user3SGDBalance.outstanding + 40
        )
      })
    })

    test('Full match order (BUY with price that is higher than ASK Price)', async ({
      investment,
      auth
    }) => {
      await test.step('The sell order should be created', async () => {
  ...
  test.describe('Between 3 users', () => {
    test('(scenario 5) Partial match 1 order and remaining BUY orders', async ({
      investment,
      auth
    }) => {
      ...
      await test.step('Check that funds have been distributed', async () => {
        ...
        expect(getBalancesBefore.user1tokenBalance.available).toEqual(
          getBalancesAfter.user1tokenBalance.available + 3
        )
        expect(getBalancesBefore.user1SGDBalance.outstanding).toEqual(
          getBalancesAfter.user1SGDBalance.outstanding - 30
        )
        expect(getBalancesBefore.user1SGDBalance.onHold).toEqual(
          getBalancesAfter.user1SGDBalance.onHold + 20
        )

        expect(getBalancesBefore.user2tokenBalance.available).toEqual(
          getBalancesAfter.user2tokenBalance.available
        )
        expect(getBalancesBefore.user2SGDBalance.outstanding).toEqual(
          getBalancesAfter.user2SGDBalance.outstanding - 27
        )
        expect(getBalancesBefore.user2SGDBalance.onHold).toEqual(
          getBalancesAfter.user2SGDBalance.onHold + 27
        )

        expect(getBalancesBefore.user3tokenBalance.available).toEqual(
          getBalancesAfter.user3tokenBalance.available - 3
        )
        expect(getBalancesBefore.user3SGDBalance.outstanding).toEqual(
          getBalancesAfter.user3SGDBalance.outstanding + 30
        )
      })
    })
    test('(scenario 6) Partial match 2 orders and 1 remaining BUY order', async ({
      investment,
      auth
    }) => {
      orderResponse = await postRequest(
      ...
      await test.step('Check that funds have been distributed', async () => {
        ...
        expect(getBalancesBefore.user1tokenBalance.available).toEqual(
          getBalancesAfter.user1tokenBalance.available + 4
        )
        expect(getBalancesBefore.user1SGDBalance.outstanding).toEqual(
          getBalancesAfter.user1SGDBalance.outstanding - 40
        )

        expect(getBalancesBefore.user2tokenBalance.available).toEqual(
          getBalancesAfter.user2tokenBalance.available + 6
        )
        expect(getBalancesBefore.user2SGDBalance.outstanding).toEqual(
          getBalancesAfter.user2SGDBalance.outstanding - 54
        )

        expect(getBalancesBefore.user3tokenBalance.available).toEqual(
          getBalancesAfter.user3tokenBalance.available - 10
        )
        expect(getBalancesBefore.user3SGDBalance.outstanding).toEqual(
          getBalancesAfter.user3SGDBalance.outstanding + 94
        )
      })
    })
  })
